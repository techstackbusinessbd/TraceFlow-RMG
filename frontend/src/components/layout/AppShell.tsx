...
export const AppShell: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col text-slate-100">
      ...
      <header className="h-16 bg-slate-900 border-b border-slate-800 flex items-center px-4 justify-between gap-4 sticky top-0 z-50">
        {/* Brand */}
        ...
        {/* User Info & Crisp Solid Red Logout Button */}
        <div className="flex items-center gap-3 shrink-0">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-64 bg-slate-900/90 border-r border-slate-800 flex flex-col shrink-0 h-[calc(100vh-4rem)] sticky top-16 overflow-y-auto">
          ...
          <div className="p-3 space-y-6 flex-1">
            {activeModule.submodules.map((group) => (
              <div key={group.id} className="space-y-1">
                ...
                <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider px-2 mb-2">
                  {group.title}
                </h3>
                    ...
                    return (
                      <Link
                        ...
                        className={`flex items-center justify-between px-2.5 py-2 rounded-md text-xs font-medium transition-colors ${
                          isItemActive
                            ? 'bg-slate-800 text-blue-400 font-semibold border-l-2 border-blue-500 shadow-sm'
          ...
          <div className="p-3 border-t border-slate-800 text-[10px] text-slate-500 flex items-center justify-between">
            ...
            <span className="text-emerald-500 font-semibold">Live System</span>
          </div>
        </aside>
        ...
        <main className="flex-1 flex flex-col min-w-0 overflow-y-auto bg-slate-950">
          ...
          <div className="h-10 border-b border-slate-800/80 bg-slate-900/40 flex items-center px-6 text-xs text-slate-400 shrink-0">
            <span className="text-slate-500">TraceFlow</span>
            <ChevronRight className="w-3.5 h-3.5 mx-1.5 text-slate-600" />
            <span className="text-slate-300 font-medium">{activeModule.title}</span>
            {activeLeafItem && (
              <>
                <ChevronRight className="w-3.5 h-3.5 mx-1.5 text-slate-600" />
                <span className="text-slate-400">{activeLeafItem.group}</span>
                <ChevronRight className="w-3.5 h-3.5 mx-1.5 text-slate-600" />
                <span className="text-blue-400 font-semibold">{activeLeafItem.item}</span>
              </>
            )}
          </div>
